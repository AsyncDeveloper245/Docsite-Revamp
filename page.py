...
def make_filepath(subdir,group,pagename):
        if subdir.endswith('.md'):
            ...
            filepath = '/'.join([group,subdir,pagename]).replace('-','').replace(' ','_')
            filepath = filepath.replace('__','_')
        return filepath
...
class Pages:
    ...
    def write_links_to_yaml(self):
        for group in self.pages.keys():
            ...
            for page in self.pages[group]:
                ...
                self.links.append({str(pageloc.split('/')[0]+"_"+pageloc.split('/')[-1].replace('.md','')):'output/dist/'+filepath})
    ...
    def write_pages(self):
        ...
        for group in self.pages.keys():
            ...
            for page in self.pages[group]:
                ...
                print(f"Writing Markdown🔄: {pagename} to {ROOT_DIR + filepath + '/index.html'}")
                try:

                    self.write_to_template("output/" + pageloc, ROOT_DIR + filepath +'/index.html', group, pagename)
                ...
                for page in self.pages["Encyclopedia"]:
                    ...
                    for c in ascii_lowercase:
                        ...
                        if letter == page_first_letter:
                            ...
                            self.encyclopedia_object[letter] = self.encyclopedia_object.get(letter,[]) + [(pagename,'/'+ROOT_DIR + filepath)]
                            self.encyclopedia_object[letter] = sorted(self.encyclopedia_object[letter])


            parse_template(self.template_objects,css = self.css_files, logo = self.logo, pagename = pagename,pedia_object = self.pedia_obj,js=self.js_files,nav_pages = self.nav_pages,encyclopedia_object=self.encyclopedia_object)
            ...
            for page in self.pages[group]:
                ...
                if not 'PDF' in pname:
                    ...
                    if group in list(nav_pages.keys()):
                            nav_pages[group].append({pname:f"{ '/' + ROOT_DIR + filepath}"})               
                    else:
                        nav_pages[group] =[]
    ...
    def write_to_template(self,markdown_path,output_path,group,pagename):
        ...
        for header in headers:
            ...
            self.search_obj.append({"name":header.text+'-'+group, "location":'/' +output_path_})
